...
class RecipeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $ingredients = [];
        foreach ($request->indgredients as $indgredient) {
            $ingredients[] = new RecipeIngredient($indgredient);
        }
        $directions = [];
        ...
        if (!$request->hasFile('image') && !$request->file('image')->isValid()) {
            return abort(404, 'Image net uploaded');
        }
        ...
        $recipe->ingrediends()->saveMany($ingredients);
        return response()->json([
            'saved' => true,
            'id' => $recipe->id,
